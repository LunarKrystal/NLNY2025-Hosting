...
function ensureConfigFile() {
	if (!fs.existsSync('./fca_setting.json')) {
		const defaultConfig = {
			autologin: {
				enable: false,
				email: "Tính năng này",
				password: "hiện chưa khả dụng",
				key2fa: "không cần thiết nhập"
			},
			autoRestartMQTT: 0
		};
		fs.writeFileSync('./fca_setting.json', JSON.stringify(defaultConfig, null, 2));
	}
}
ensureConfigFile();
global.fca.config = require(process.cwd()+'/fca_setting.json');
...
function buildAPI(globalOptions, html, jar) {
	...
	if (!maybeCookie) {
		throw { error: "Tài khoản Facebook bạn xảy ra sự cố. hãy kiểm tra lại!" };
	}
	...
	log.info("login", `Đã đăng nhập tại ${userID}`);
	...
	try {
		...
		log.info('login', `Vùng tài khoản: ${region}`);
	} catch (e) {
		log.warning('login', 'Lỗi khi lấy vùng tài khoản !');
	}
	const tokenMatch = html.match(/DTSGInitialData.*?token":"(.*?)"/);
	if (tokenMatch) {
		fb_dtsg = tokenMatch[1];
		if (fb_dtsg) {
			//log.info(`login`, `Đã tìm thấy appstate.json`);
		}
	}
	const ctx = {
		userID: userID,
