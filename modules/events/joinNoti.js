...
module.exports.onLoad = function() {
  ...
  if (!existsSync(path2)) mkdirSync(path2, {
      recursive: true
  });
...
module.exports.run = async function({
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
      ...
      const fs = require("fs");
      var mlg = "Kết nối thành công\nĐã load toàn bộ lệnh và người dùng trong nhóm.\n❌ Nếu nhóm của bạn chưa kích hoạt sử dụng bot, vui lòng sử dụng lệnh '${prefix}callad' để liên hệ Admin.\n─────────────────\n🌐 Facebook: https://www.facebook.com/685760301"
      return api.sendMessage(threadID, async () => {
          await api.shareContact(`${mlg}`, 685760301, threadID);
      });
